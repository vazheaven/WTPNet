...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)        # [B, 24, 3, H/4, W/4]
        x = self.features[:4](x)
        outputs['stage2'] = x
...
def build_3d_cnn(cfg, pretrained=False):
    if 'resnet' in cfg['backbone_td']:
        model, feat_dims = build_resnet_3d(model_name=cfg['backbone_td'], pretrained=pretrained)
    elif 'resnext' in cfg['backbone_td']:
        model, feat_dims = build_resnext_3d(model_name=cfg['backbone_td'], pretrained=pretrained)
    elif 'shufflenetv2' in cfg['backbone_td']:
...
class BackboneTD(nn.Module):
    def __init__(self, cfg, pretrained=False):
        super(BackboneTD, self).__init__()
        self.cfg = cfg
